...
Adafruit_2801Matrix::Adafruit_2801Matrix(int w, int h, uint8_t pin1, uint8_t pin2,
  uint8_t matrixType, uint8_t ledType) : Adafruit_GFX(w, h),
  Adafruit_WS2801(w, h, pin1, pin2, ledType), type(matrixType), matrixWidth(w),
  matrixHeight(h), tilesX(0), tilesY(0), remapFn(NULL) { }

// Constructor for tiled matrices:
Adafruit_2801Matrix::Adafruit_2801Matrix(uint8_t mW, uint8_t mH, uint8_t tX,
  uint8_t tY, uint8_t pin1, uint8_t pin2, uint8_t matrixType, uint8_t ledType) :
  Adafruit_GFX(mW * tX, mH * tY), Adafruit_WS2801(mW * mH * tX * tY, pin1, pin2,
...
uint16_t Adafruit_2801Matrix::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint16_t)(r & 0xF8) << 8) |
         ((uint16_t)(g & 0xFC) << 3) |
                    (b         >> 3);
}

void Adafruit_2801Matrix::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_2801Matrix::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c24;
...
void Adafruit_2801Matrix::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
